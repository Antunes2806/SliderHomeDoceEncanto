...
export default function CupcakeDocedeLeite() {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        ...
        const imageRef = ref(storage, "cupcakedl.png");
    ...
    fetchImage(); // Chama a função para buscar a imagem
  }, []);

  const handleToggleFavorite = async () => {
    const currentUser = auth.currentUser;
    if (!currentUser) {
      setShowAlert(true); // Exibe o alerta se o usuário não estiver logado
      return;
    }
  ...
  const handleAddToCart = () => {
    ...
    if (item) {
      dispatch(addToCart(item)); // Adiciona o item ao carrinho
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.txtcupcake}>
        ...
        generoso de doce de leite, coroado com uma cobertura que derrete na
        boca!
      </Text>
