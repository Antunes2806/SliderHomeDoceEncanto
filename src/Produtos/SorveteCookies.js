...
export default function SorveteCookies() {
  ...
  const itemfav = {
    id: "4", // Exemplo de ID do produto
    name: "Sorvete Cookies",
    valor: 15.0,
    description:
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        ...
        const imageRef = ref(storage, "svtcookie.png");
  ...
  const handleToggleFavorite = async () => {
    ...
    if (!currentUser) {
      setShowAlert(true); // Mostra o alerta caso o usuário não esteja logado
      return;
    }
